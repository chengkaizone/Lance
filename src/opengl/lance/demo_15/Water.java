...
import static opengl.lance.demo_15.Constant_8.*;
...
public class Water {
	...
	public Water(int texId, double startAngle, int rows, int cols, int bei) {
		this.texId = texId;
		...
		for (int j = 0; j < rows; j++) {
			for (int i = 0; i < cols; i++) {
				...
				float zsy = (float) Math.sin(startAngle + i * angleSpan) * 0.4f
						- WATER_HIGH_ADJUST;
				// 计算右边一列的y坐标
				float zsyp = (float) Math.sin(startAngle + (i + 1) * angleSpan)
