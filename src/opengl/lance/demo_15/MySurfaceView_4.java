...
public class MySurfaceView_4 extends GLSurfaceView {
	...
	ArrayList<LogicControl> al;// 椎体控制列表
	GoThread gt;// 运动线程
	...
	public MySurfaceView_4(Context context) {
		...
		al = new ArrayList<LogicControl>();// 创建对象
	}
	...
	private class SceneRenderer implements GLSurfaceView.Renderer {
		...
		public void onSurfaceCreated(GL10 gl, EGLConfig config) {
			...
			// ct=new
			// LogicControl(zhuiTi,0,0,-7*UNIT_OFFSET_Z,0.0f,0.0f,0.0f);//Z轴上的障碍物
			// al.add(ct);
			ct = new LogicControl(zhuiTi_T, UNIT_OFFSET_X, 0, -UNIT_OFFSET_Z,
					0.0f, 0.0f, 0.0f);// X轴上的障碍物
			al.add(ct);
			// ct=new
			// LogicControl(zhuiTi,0,UNIT_OFFSET_Y,-UNIT_OFFSET_Z,0.0f,0.0f,0.0f);//Y轴上的障碍物
			// al.add(ct);
			...
			// al.add(ct);
			// ct=new
			// LogicControl(zhuiTi,0,0,4*UNIT_OFFSET_Z,0.0f,0.0f,0.0f);//Z轴上的障碍物
			// al.add(ct);//加入列表中
