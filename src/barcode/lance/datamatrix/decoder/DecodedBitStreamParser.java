...
import barcode.lance.common.BitSource;
import barcode.lance.common.DecoderResult;

/**
 * <p>
 * Data Matrix Codes can encode text as bits in one of several modes, and can
 * use multiple modes in one Data Matrix Code. This class decodes the bits back
 * into text.
 * </p>
 * 
 * <p>
...
final class DecodedBitStreamParser {
	 ...
	 */
	private static int decodeAsciiSegment(BitSource bits, StringBuffer result,
			StringBuffer resultTrailer) throws FormatException {
		...
		do {
			...
			if (oneByte == 0) {
				...
				return PAD_ENCODE;
			} else if (oneByte <= 229) { // 2-digit data 00-99 (Numeric Value +
											// 130)
				...
				// throw ReaderException.getInstance();
				// Ignore this symbol for now
			} else if (oneByte == 235) { // Upper Shift (shift to Extended
											// ASCII)
				upperShift = true;
				...
				// (bbrown): I think we need to support ECI
				// throw ReaderException.getInstance();
				// Ignore this symbol for now
	 ...
	 */
	private static void decodeC40Segment(BitSource bits, StringBuffer result)
			throws FormatException {
		// Three C40 values are encoded in a 16-bit value as
		// (1600 * C1) + (40 * C2) + C3 + 1
		// (bbrown): The Upper Shift with C40 doesn't work in the 4 value
	 ...
	 */
	private static void decodeTextSegment(BitSource bits, StringBuffer result)
			throws FormatException {
		// Three Text values are encoded in a 16-bit value as
		// (1600 * C1) + (40 * C2) + C3 + 1
		// (bbrown): The Upper Shift with Text doesn't work in the 4 value
		// scenario all the time
		boolean upperShift = false;
	 ...
	 */
	private static void decodeAnsiX12Segment(BitSource bits, StringBuffer result)
			throws FormatException {
		...
		do {
			...
			for (int i = 0; i < 4; i++) {
				...
				if (edifactValue == 0x1F) { // 011111
					unlatch = true;
					// If we encounter the unlatch code then continue reading
					// because the Codeword triple
					// is padded with 0's
				}

				if (!unlatch) {
					if ((edifactValue & 32) == 0) { // no 1 in the leading (6th)
													// bit
						edifactValue |= 64; // Add a leading 01 to the 6 bit
											// binary value
					}
	...
	private static void decodeBase256Segment(BitSource bits,
			StringBuffer result, Vector byteSegments) throws FormatException {
