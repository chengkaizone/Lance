...
import barcode.lance.common.StringUtils;
...
final class DecodedBitStreamParser {
			...
			if (!mode.equals(Mode.TERMINATOR)) {
				if (mode.equals(Mode.FNC1_FIRST_POSITION)
					...
					if (mode.equals(Mode.HANZI)) {
						...
						int countHanzi = bits.readBits(mode
	 ...
	 */
	private static void decodeHanziSegment(BitSource bits, StringBuffer result,
		...
		while (count > 0) {
			...
			int assembledTwoBytes = ((twoBytes / 0x060) << 8)
					| (twoBytes % 0x060);
