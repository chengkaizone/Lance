package barcode.lance.qrcode.detector;
...
import barcode.lance.common.PerspectiveTransform;
import barcode.lance.qrcode.decoder.Version;
...
public class QRDetector {
	...
	protected static int computeDimension(ResultPoint topLeft,
			...
			throws NotFoundException {
		int tltrCentersDimension = round(ResultPoint
				.distance(topLeft, topRight) / moduleSize);
		int tlblCentersDimension = round(ResultPoint.distance(topLeft,
	...
	private float sizeOfBlackWhiteBlackRunBothWays(int fromX, int fromY,
		...
		if (otherToX < 0) {
			...
			scale = (float) (image.getWidth() - fromX)
		...
		if (otherToY < 0) {
			...
			scale = (float) (image.getHeight() - fromY)
	...
	private float sizeOfBlackWhiteBlackRun(int fromX, int fromY, int toX,
		...
		int state = 0; // In black pixels, looking for white, first or second
						// time
		for (int x = fromX, y = fromY; x != toX; x += xstep) {
			...
			if (state == 3) { // Found black, white, black, and stumbled back
								// onto white; done
				int diffX = x - fromX;
				int diffY = y - fromY;
				if (xstep < 0) {
				...
				return (float) Math.sqrt((double) (diffX * diffX + diffY
						* diffY));
			}
			error += dy;
			if (error > 0) {
