...
final class DecodedBitStreamParser {
	...
	static DecoderResult decode(int[] codewords) throws FormatException {
		...
		while (codeIndex < codewords[0]) {
			...
			default: {
				...
				// appeared to be missing the starting mode. In these cases
				// defaulting
		...
		while ((codeIndex < codewords[0]) && !end) {
			...
			if (code < TEXT_COMPACTION_MODE_LATCH) {
				...
				case MODE_SHIFT_TO_BYTE_COMPACTION_MODE: {
					...
					// This switch shall be in effect for only the next
					// codeword,
					// after which the mode shall revert to the prevailing
					// sub-mode
					// of the Text Compaction mode. Codeword 913 is only
					// available
	...
	private static void decodeTextCompaction(int[] textCompactionData,
		...
		// Compaction mode Alpha sub-mode (uppercase alphabetic). A latch
		// codeword from another mode to the Text
		// Compaction mode shall always switch to the Text Compaction Alpha
		// sub-mode.
		int subMode = ALPHA;
	 ...
	 * 
	 * @param value1
	 *            Any number represented as a string.
	 * @param value2
	 *            A number <= 999.
