...
public final class PDFDecoder {
	...
	public DecoderResult decode(BitMatrix bits) throws FormatException {
		// Construct a parser to read the data codewords and error-correction
		// level
	...
	private static void verifyCodewordCount(int[] codewords, int numECCodewords)
		...
		// The first codeword, the Symbol Length Descriptor, shall always encode
		// the total number of data
		// codewords in the symbol, including the Symbol Length Descriptor
		// itself, data codewords and pad
		...
		if (numberOfCodewords == 0) {
			// Reset to the length of the array - 8 (Allow for at least level 3
			// Error Correction (8 Error Codewords)
