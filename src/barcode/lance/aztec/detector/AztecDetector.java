package barcode.lance.aztec.detector;

import barcode.lance.assist.NotFoundException;
import barcode.lance.assist.ResultPoint;
import barcode.lance.aztec.AztecDetectorResult;
import barcode.lance.common.BitMatrix;
import barcode.lance.common.GridSampler;
import barcode.lance.common.detector.WhiteRectangleDetector;
import barcode.lance.common.reedsolomon.GenericGF;
import barcode.lance.common.reedsolomon.ReedSolomonDecoder;
import barcode.lance.common.reedsolomon.ReedSolomonException;
 ...
 */
public final class AztecDetector {
	...
	public AztecDetector(BitMatrix image) {
		this.image = image;
	}
	 ...
	 * </p>
	 * 
	 * @return {@link AztecDetectorResult} encapsulating results of detecting an
	 *         Aztec Code
	...
	public AztecDetectorResult detect() throws NotFoundException {
		...
		// 5. Sample the grid
		BitMatrix bits = sampleGrid(image, corners[shift % 4],
				corners[(shift + 3) % 4], corners[(shift + 2) % 4],
				corners[(shift + 1) % 4]);

		return new AztecDetectorResult(bits, corners, compact, nbDataBlocks,
				nbLayers);
	}

	/**
	 * <p>
	 * Extracts the number of data layers and data blocks from the layer around
	 * the bull's eye
	 * </p>
	 * 
	 * @param bullEyeCornerPoints
	...
	private void extractParameters(Point[] bullEyeCornerPoints)
		...
		boolean[] resab = sampleLine(bullEyeCornerPoints[0],
				bullEyeCornerPoints[1], 2 * nbCenterLayers + 1);
		boolean[] resbc = sampleLine(bullEyeCornerPoints[1],
				bullEyeCornerPoints[2], 2 * nbCenterLayers + 1);
		boolean[] rescd = sampleLine(bullEyeCornerPoints[2],
				bullEyeCornerPoints[3], 2 * nbCenterLayers + 1);
		boolean[] resda = sampleLine(bullEyeCornerPoints[3],
				bullEyeCornerPoints[0], 2 * nbCenterLayers + 1);
	 ...
	 * @throws NotFoundException
	 *             if the corner points do not fit in the image
	 */
	private ResultPoint[] getMatrixCornerPoints(Point[] bullEyeCornerPoints)
		...
		return new ResultPoint[] { new ResultPoint(targetax, targetay),
	 ...
	 */
	private static void correctParameterData(boolean[] parameterData,
	...
	private Point[] getBullEyeCornerPoints(Point pCenter)
		...
		for (nbCenterLayers = 1; nbCenterLayers < 9; nbCenterLayers++) {
			...
			if (nbCenterLayers > 2) {
				...
				if (q < 0.75 || q > 1.25
						|| !isWhiteOrBlackRectangle(pouta, poutb, poutc, poutd)) {
					break;
				}
			}
	...
	private Point getMatrixCenter() {
		...
		// Get a white rectangle that can be the border of the matrix in center
		// bull's eye or
		try {
			...
			pointA = getFirstDifferent(new Point(cx + 15 / 2, cy - 15 / 2),
					false, 1, -1).toResultPoint();
			pointB = getFirstDifferent(new Point(cx + 15 / 2, cy + 15 / 2),
					false, 1, 1).toResultPoint();
			pointC = getFirstDifferent(new Point(cx - 15 / 2, cy + 15 / 2),
					false, -1, 1).toResultPoint();
			pointD = getFirstDifferent(new Point(cx - 15 / 2, cy - 15 / 2),
					false, -1, -1).toResultPoint();

		}

		// Compute the center of the rectangle
		int cx = round((pointA.getX() + pointD.getX() + pointB.getX() + pointC
				.getX()) / 4);
		int cy = round((pointA.getY() + pointD.getY() + pointB.getY() + pointC
				.getY()) / 4);
		...
		try {
			ResultPoint[] cornerPoints = new WhiteRectangleDetector(image, 15,
					cx, cy).detect();
			pointA = cornerPoints[0];
			pointB = cornerPoints[1];
			pointC = cornerPoints[2];
		...
		cx = round((pointA.getX() + pointD.getX() + pointB.getX() + pointC
				.getX()) / 4);
		cy = round((pointA.getY() + pointD.getY() + pointB.getY() + pointC
				.getY()) / 4);
	...
	private BitMatrix sampleGrid(BitMatrix image, ResultPoint topLeft,
		...
		return sampler.sampleGrid(image, dimension, dimension, 0.5f, 0.5f,
	...
	private void getParameters(boolean[] parameterData) {
		...
		for (int i = nbBitsForNbLayers; i < nbBitsForNbLayers
				+ nbBitsForNbDatablocks; i++) {
			nbDataBlocks <<= 1;
			if (parameterData[i]) {
				nbDataBlocks += 1;
			}
	 ...
	 * 
	 * @return 1 if segment more than 90% black, -1 if segment is more than 90%
	 *         white, 0 else
	 */
