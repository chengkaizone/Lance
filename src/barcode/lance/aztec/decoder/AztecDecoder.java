package barcode.lance.aztec.decoder;
...
/**
 * <p>
 * The main class which implements Aztec Code decoding -- as opposed to locating
 * and extracting the Aztec Code from an image.
 * </p>
 * 
 * @author David Olivier
 */
public final class AztecDecoder {
	...
	private static final int[] NB_BITS = { 0, 128, 288, 480, 704, 960, 1248,
	...
	private static final int[] NB_DATABLOCK = { 0, 21, 48, 60, 88, 120, 156,
	...
	private static final String[] MIXED_TABLE = { "CTRL_PS", " ", "\1", "\2",
			"\3", "\4", "\5", "\6", "\7", "\b", "\t", "\n", "\13", "\f", "\r",
			"\33", "\34", "\35", "\36", "\37", "@", "\\", "^", "_", "`", "|",
	 ...
	 */
	private String getEncodedData(boolean[] correctedBits)
			throws FormatException {
	...
	/**
	 * 
	 * Gets the character (or string) corresponding to the passed code in the
	 * given table
	 * 
	 * @param table
	 *            the table used
	...
	private boolean[] correctBits(boolean[] rawbits) throws FormatException {
		...
		if (ddata.isCompact()) {
			offset = NB_BITS_COMPACT[ddata.getNbLayers()] - numCodewords
					* codewordSize;
			numECCodewords = NB_DATABLOCK_COMPACT[ddata.getNbLayers()]
					- numDataCodewords;
		} else {
			offset = NB_BITS[ddata.getNbLayers()] - numCodewords * codewordSize;
			numECCodewords = NB_DATABLOCK[ddata.getNbLayers()]
					- numDataCodewords;
		}
	 ...
	 * @throws FormatException
	 *             if the matrix is not a valid aztec code
	 */
	private boolean[] extractBits(BitMatrix matrix) throws FormatException {
		...
		while (layer != 0) {
			...
			for (int i = 2 * size + 1; i > 5; i--) {
				rawbits[rawbitsOffset + 4 * size - 8 + (2 * size - i) + 1] = matrix
						.get(matrixOffset + size - 1 - flip, matrixOffset + i
								/ 2 - 1);
				rawbits[rawbitsOffset + 6 * size - 12 + (2 * size - i) + 1] = matrix
						.get(matrixOffset + i / 2 - 1, matrixOffset + flip);
				flip = (flip + 1) % 2;
			}
