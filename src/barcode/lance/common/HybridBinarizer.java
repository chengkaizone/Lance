...
import barcode.lance.assist.Binarizer;
import barcode.lance.assist.LuminanceSource;
...
public final class HybridBinarizer extends GlobalHistogramBinarizer {
	...
	// This class uses 5x5 blocks to compute local luminance, where each block
	// is 8x8 pixels.
	...
	// constructor instead, but there are some advantages to doing it lazily,
	// such as making
	// profiling easier, and not doing heavy lifting when callers don't expect
	// it.
	private void binarizeEntireImage() throws NotFoundException {
		if (matrix == null) {
			...
			if (source.getWidth() >= MINIMUM_DIMENSION
				...
				int[][] blackPoints = calculateBlackPoints(luminances,
						subWidth, subHeight, width, height);

				matrix = new BitMatrix(width, height);
				calculateThresholdForBlock(luminances, subWidth, subHeight,
	...
	// For each 8x8 block in the image, calculate the average black point using
	// a 5x5 grid
	// of the blocks around it. Also handles the corner cases (fractional blocks
	// are computed based
	// on the last 8 pixels in the row/column which are also used in the
	// previous block).
	private static void calculateThresholdForBlock(byte[] luminances,
		...
		for (int y = 0; y < subHeight; y++) {
			...
			for (int x = 0; x < subWidth; x++) {
				...
				int average = sum / 25;
				threshold8x8Block(luminances, xoffset, yoffset, average, width,
						matrix);
			}
		}
	}
	...
	private static void threshold8x8Block(byte[] luminances, int xoffset,
	...
	private static int[][] calculateBlackPoints(byte[] luminances,
		...
		for (int y = 0; y < subHeight; y++) {
			...
			for (int x = 0; x < subWidth; x++) {
				...
				// If the contrast is inadequate, use half the minimum, so that
				// this block will be
				// treated as part of the white background, but won't drag down
				// neighboring blocks
				// too much.
				int average;
				if (max - min > 24) {
