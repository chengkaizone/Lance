...
/**
 * An abstract class to be implemented by the chart rendering classes. 抽象的图表类
 * 用于被渲染类实现
 * 
 * @author lance
 * 
...
public abstract class AbstractChart implements Serializable {
	/**
	 * The graphical representation of the chart. 绘制图形表示图
	 * 
	 * @param canvas
	...
	/**
	 * Draws the chart background. 绘制图表背景
	 * 
	 * @param renderer
	 *            the chart renderer
	 * @param canvas
	...
	/**
	 * Draws the chart legend. 绘制图表中的刻印文字
	 * 
	 * @param canvas
	 *            the canvas to paint to
	 * @param renderer
	 ...
	 */
	protected int drawLegend(Canvas canvas, DefaultRenderer renderer,
			String[] titles, int left, int right, int y, int width, int height,
			int legendSize, Paint paint, boolean calculate) {
		float size = 32;
	...
	/**
	 * Draw a multiple lines string. 绘制多行字符串
	 * 
	 * @param canvas
	 *            the canvas to paint to
	 * @param text
	 ...
	 *            the paint to be used for drawing
	 */
	protected void drawString(Canvas canvas, String text, float x, float y,
			Paint paint) {
		if (text != null) {
	...
	/**
	 * Calculates if the current width exceeds the total width. 如果当前的计算宽度超过总宽度
	 * 
	 * @param currentWidth
	 *            the current width
	 * @param renderer
	 ...
	 */
	protected boolean getExceed(float currentWidth, DefaultRenderer renderer,
			int right, int width) {
	...
	/**
	 * Checks if the current chart is rendered as vertical. 检查当前图表是否垂直呈现
	 * 
	 * @param renderer
	 *            the renderer
	 ...
	 * @return
	 */
	private static float[] calculateDrawPoints(float p1x, float p1y, float p2x,
			float p2y, int screenHeight, int screenWidth) {
		float drawP1x;
		float drawP1y;
		float drawP2x;
		float drawP2y;
		...
		if (p1y > screenHeight) {
			...
			if (drawP1x < 0) {
				// If Intersection is left of the screen we calculate the
				// intersection
				// with the left border
				drawP1x = 0;
				drawP1y = p1y - m * p1x;
			} else if (drawP1x > screenWidth) {
				// If Intersection is right of the screen we calculate the
				// intersection
	...
	/**
	 * The graphical representation of a path. 绘制图形路径
	 * 
	 * @param canvas
	 *            the canvas to paint to
	 * @param points
	...
	protected void drawPath(Canvas canvas, float[] points, Paint paint,
		...
		tempDrawPoints = calculateDrawPoints(points[0], points[1], points[2],
				points[3], height, width);
		...
		for (int i = 4; i < points.length; i += 2) {
			...
			tempDrawPoints = calculateDrawPoints(points[i - 2], points[i - 1],
					points[i], points[i + 1], height, width);
			if (!circular) {
				path.moveTo(tempDrawPoints[0], tempDrawPoints[1]);
			}
			path.lineTo(tempDrawPoints[2], tempDrawPoints[3]);
	...
	/**
	 * Returns the legend shape width. 返回刻文形状宽度
	 * 
	 * @param seriesIndex
	 *            the series index
	...
	/**
	 * The graphical representation of the legend shape. 绘制刻纹形状
	 * 
	 * @param canvas
	 *            the canvas to paint to
	 * @param renderer
	...
	/**
	 * Calculates the best text to fit into the available space. 在一个可用的空间计算最佳文本
	 * 
	 * @param text
	 * @param width
	 * @param paint
	...
	/**
	 * Calculates the current legend size. 计算当前刻纹字体大小
	 * 
	 * @param renderer
	 *            the renderer
	 * @param defaultHeight
	...
	protected void drawLabel(Canvas canvas, String labelText,
			...
			int centerX, int centerY, float shortRadius, float longRadius,
			float currentAngle, float angle, int left, int right, int color,
		...
		if (renderer.isShowLabels()) {
			...
			while (!okBounds && line) {
				...
				for (int j = 0; j < length && !intersects; j++) {
					...
					if (prevLabelBounds.intersects(xLabel, yLabel, xLabel
							+ widthLabel, yLabel + size)) {
						intersects = true;
			...
			if (line) {
				prevLabelsBounds.add(new RectF(xLabel, yLabel, xLabel
						+ widthLabel, yLabel + size));
			}
		}
	}
	 ...
	 * coordinates, null is returned. 根据屏幕坐标返回一个块
	 * 
	 * @param screenPoint
	 * @return
	 */
