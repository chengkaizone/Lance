...
import org.lance.chartengine.model.MultipleCategorySeries;
...
/**
 * The doughnut chart rendering class. 环形图表
 * 
 * @author lance
 * 
 */
public class DoughnutChart extends RoundChart {
	...
	/**
	 * Builds a new doughnut chart instance. 创建实例
	 * 
	 * @param dataset
	 *            the series dataset
	 * @param renderer
	...
	/**
	 * The graphical representation of the doughnut chart. 绘制环形图表
	 * 
	 * @param canvas
	...
	public void draw(Canvas canvas, int x, int y, int width, int height,
		...
		if (mRenderer.isFitLegend()) {
			legendSize = drawLegend(canvas, mRenderer, categories, left, right,
					y, width, height, legendSize, paint, true);
		}
		...
		for (int category = 0; category < cLength; category++) {
			...
			RectF oval = new RectF(mCenterX - radius, mCenterY - radius,
			...
			for (int i = 0; i < sLength; i++) {
				...
				drawLabel(canvas, mDataset.getTitles(category)[i], mRenderer,
			...
			paint.setStyle(Style.FILL);
			oval = new RectF(mCenterX - radius, mCenterY - radius, mCenterX
