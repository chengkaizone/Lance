...
import org.lance.chartengine.renderer.DialRenderer;
import org.lance.chartengine.renderer.DialRenderer.Type;
...
/**
 * The dial chart rendering class. 刻度盘图表
 * 
 * @author lance
 * 
 */
public class DialChart extends RoundChart {
	...
	/**
	 * Builds a new dial chart instance. 创建实例
	 * 
	 * @param dataset
	 *            the series dataset
	 * @param renderer
	...
	/**
	 * The graphical representation of the dial chart. 绘制刻度盘图表
	 * 
	 * @param canvas
	...
	public void draw(Canvas canvas, int x, int y, int width, int height,
		...
		drawTicks(canvas, min, max, angleMin, angleMax, mCenterX, mCenterY,
		...
		int count = mRenderer.getSeriesRendererCount();
		for (int i = 0; i < count; i++) {
			double angle = getAngleForValue(mDataset.getValue(i), angleMin,
					angleMax, min, max);
			paint.setColor(mRenderer.getSeriesRendererAt(i).getColor());
			boolean type = mRenderer.getVisualTypeForIndex(i) == Type.ARROW;
			drawNeedle(canvas, angle, mCenterX, mCenterY, shortRadius, type,
					paint);
		}
	...
	/**
	 * Returns the angle for a specific chart value. 根据指定的值返回角度
	 * 
	 * @param value
	 *            the chart value
	 * @param minAngle
	...
	/**
	 * Draws the chart tick lines. 绘制表的刻度线
	 * 
	 * @param canvas
	 *            the canvas
	 * @param min
	...
	/**
	 * Returns the angle for a specific chart value. 绘制指针
	 * 
	 * @param canvas
	 *            the canvas
	 * @param angle
