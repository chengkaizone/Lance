...
/**
 * The range bar chart rendering class. 范围栏图表
 * 
 * @author lance
 * 
 */
public class RangeBarChart extends BarChart {
	...
	/**
	 * Builds a new range bar chart instance. 创建范围栏实例
	 * 
	 * @param dataset
	...
	/**
	 * The graphical representation of a series. 绘制图形数据
	 * 
	 * @param canvas
	...
	public void drawSeries(Canvas canvas, Paint paint, float[] points,
		...
		for (int i = start; i < length; i += 4) {
			if (points.length > i + 3) {
				...
				drawBar(canvas, xMin, yMin, xMax, yMax, halfDiffX, seriesNr,
						seriesIndex, paint);
			}
		}
		paint.setColor(seriesRenderer.getColor());
	...
	/**
	 * The graphical representation of the series values as text. 为图表绘制文本
	 * 
	 * @param canvas
	 *            the canvas to paint to
	 * @param series
		...
		for (int i = start; i < points.length; i += 4) {
			...
			if (!isNullValue(series.getY(index + 1)) && points.length > i + 3) {
				...
				drawText(canvas, getLabel(series.getY(index + 1)), x,
						points[i + 3] - renderer.getChartValuesSpacing(),
						paint, 0);
			}
			if (!isNullValue(series.getY(index)) && points.length > i + 1) {
				...
				drawText(canvas, getLabel(series.getY(index)), x,
						points[i + 1] + renderer.getChartValuesTextSize()
								+ renderer.getChartValuesSpacing() - 3, paint,
						0);
			}
		}
	}
