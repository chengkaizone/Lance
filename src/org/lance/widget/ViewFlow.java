...
public class ViewFlow extends AdapterView<Adapter> {
	...
	// 默认缓存数量---如果缓存的数量大于或小于适配器中的数量的换回退得到的视图可能重复
	...
	private OnViewSwitchListener switchListener;
	...
	public ViewFlow(Context context, AttributeSet attrs, int defStyle) {
	...
	public void setOnSwitchListener(OnViewSwitchListener switchListener) {
		...
		if (switchListener != null) {
			if (mSideBuffer >= 1) {
				switchListener.onSwitched(
						mLoadedViews.get(mCurrentBufferIndex),
	 ...
	 * 视图切换监听器
	 */
	public static interface OnViewSwitchListener {
